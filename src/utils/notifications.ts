...
export interface EmailNotification {
  ...
  subject: string;
  body: string;
  type: 'match_found' | 'item_claimed' | 'reminder';
}

export function generateMatchNotification(
  match: Match,
  lostItem: Item,
  foundItem: Item
): EmailNotification[] {
...
function generateLostItemMatchEmail(match: Match, lostItem: Item, foundItem: Item): string {
...
function generateFoundItemMatchEmail(match: Match, lostItem: Item, foundItem: Item): string {
  const matchPercentage = Math.round(match.matchScore * 100);
...
export function generateReminderEmail(item: Item): EmailNotification {
  const daysSinceReported = Math.floor(
    (new Date().getTime() - item.dateReported.getTime()) / (1000 * 60 * 60 * 24)
  );
