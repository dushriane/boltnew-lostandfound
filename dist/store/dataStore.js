...
exports.useDataStore = (0, zustand_1.create)()((0, middleware_1.persist)((set, get) => ({
    items: mockData_1.mockItems,
    matches: [],
    notifications: [],
    payments: [],
    loading: false,
    addItem: async (itemData) => {
        ...
        setTimeout(() => {
            ...
            const newMatches = (0, matching_1.findMatches)(currentState.items);
            const addedMatches = newMatches.filter(match => !currentState.matches.find(m => m.id === match.id));
            if (addedMatches.length > 0) {
                ...
                addedMatches.forEach(async (match) => {
                    ...
                    if (lostItem && foundItem) {
                        const notifications = (0, notifications_1.generateMatchNotification)(match, lostItem, foundItem);
                        for (const notification of notifications) {
                            await (0, notifications_1.sendEmailNotification)(notification);
                        ...
                        set(state => ({
                            matches: state.matches.map(m => m.id === match.id ? { ...m, notificationSent: true } : m)
                        }));
                    }
                });
            }
    ...
    deleteItem: (id) => {
        set(state => ({
            ...
            matches: state.matches.filter(match => match.lostItemId !== id && match.foundItemId !== id)
        }));
    },
    ...
    confirmMatch: (matchId) => {
        ...
        if (match) {
            set(state => ({
                matches: state.matches.map(m => m.id === matchId ? { ...m, status: 'confirmed' } : m),
                items: state.items.map(item => {
                    if (item.id === match.lostItemId || item.id === match.foundItemId) {
                        return { ...item, status: 'matched', matchedWith: item.id === match.lostItemId ? match.foundItemId : match.lostItemId
                        };
                    }
                    return item;
                })
    ...
    getMatchesForUser: (userId) => {
        ...
        return get().matches.filter(match => userItemIds.includes(match.lostItemId) || userItemIds.includes(match.foundItemId));
    },
    ...
    verifyItem: (id, adminNotes) => {
        set(state => ({
            items: state.items.map(item => item.id === id ? { ...item, isVerified: true, adminNotes } : item)
        }));
    },
    getAdminStats: () => {
        const state = get();
        return {
            ...
            recentActivity: {
                newItems: state.items.filter(item => new Date().getTime() - item.dateReported.getTime() < 24 * 60 * 60 * 1000).length,
                newMatches: state.matches.filter(match => new Date().getTime() - match.dateMatched.getTime() < 24 * 60 * 60 * 1000).length,
            }
        };
    },
}), {
