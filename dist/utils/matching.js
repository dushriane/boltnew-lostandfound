...
function calculateMatchScore(lostItem, foundItem) {
    ...
    if (lostItem.imageEmbeddings && foundItem.imageEmbeddings &&
        ...
        const imageSimilarity = aiService_1.aiService.calculateImageSimilarity(lostItem.imageEmbeddings, foundItem.imageEmbeddings);
    ...
    const daysDifference = Math.abs((lostItem.dateOccurred.getTime() - foundItem.dateOccurred.getTime()) / (1000 * 60 * 60 * 24));
    if (daysDifference <= 7) {
    ...
    lostItems.forEach(lostItem => {
        foundItems.forEach(foundItem => {
            ...
            if (score >= MINIMUM_MATCH_SCORE) {
                ...
                if (lostItem.imageEmbeddings && foundItem.imageEmbeddings) {
                    imageMatchScore = aiService_1.aiService.calculateImageSimilarity(lostItem.imageEmbeddings, foundItem.imageEmbeddings);
                }
                matches.push({
...
function calculateAIConfidence(lostItem, foundItem) {
...
function getMatchedFields(lostItem, foundItem) {
    const fields = [];
    if (lostItem.category.toLowerCase() === foundItem.category.toLowerCase()) {
